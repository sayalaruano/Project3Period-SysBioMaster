---
title: "GSE116250 Data Processing"
...
gx_data_all <- read.delim("GSE116250_rpkm.txt.gz", as.is = T)
gx_data <- subset(gx_data_all, select = -c(Common_name)) %>% distinct(Gene, .keep_all=TRUE)
#duplicate row names with different counts  --> might have ommited important data 
rownames(gx_data) <- gx_data$Gene 
gx_data <- subset(gx_data, select = -c(Gene)) #clean this up
...
## Normalization - doesn't quite work
```{r}
...
gx_cpm[gx_cpm < -10] = NA
write.table(gx_cpm, file = "GSE116260_CPM.txt", sep = "\t", row.names = T)
